...
exports.run = (client, message, args) => {
 message.delete();
    message.guild.createChannel(`talep-${message.author.username}`, 'text').then(ch => {
        ...
        .setAuthor("» RexSky BOT| Destek Sistemi")
        .setDescription("Buradaki destek ekibimiz sizinle ilgilenecektir.\nDestek talebini iptal etmek için 'r!kapat' yazabilirsin!")
        .setFooter("RexSky BOT| Destek Sistemi")
        .setTimestamp()
        ch.send(embed).catch()
        ch.send("@here")
        ch.awaitMessages((msg)=> {
            if (msg.content === "r!kapat") {
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['destek', "destek-talebi", "destektalebi", "Destek"],
  permLevel: 0
};

exports.help = {
  name: 'destek',
  description: 'Destek Sunucusunun Davet Linkini Verir.',
  usage: 'r!destek'
};
